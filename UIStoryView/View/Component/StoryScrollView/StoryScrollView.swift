...
public class StoryScrollView: UIScrollView, UIScrollViewDelegate
{
    fileprivate let maxAngle: CGFloat = 60.0
    
    fileprivate var childViews = [UIView]()
    
    public var onDidChange: ((Int) -> Void)?;
...
//        let page = Int((scrollView.contentOffset.x + (CGFloat(0.5) * width)) / width);
//        print("end scroll animate\(page)");
////        self.onDidChange?(page);
    }
}
